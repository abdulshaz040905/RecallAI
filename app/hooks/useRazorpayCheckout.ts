  ...
  const checkout = useCallback(
    async (plan: PlanId) => {
      ...
      try {
        ...
        // Show test-card details only when Razorpay is in test mode
        if (String(data.keyId).startsWith("rzp_test_")) {
          window.alert(
...
After continuing, select “Success” on the test payment page.
If prompted for otp just add a random otp
`,
          );
        }
